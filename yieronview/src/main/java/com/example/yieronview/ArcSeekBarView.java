...
public class ArcSeekBarView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ArcSeekBarView);
        mArcColors = getArcColors(context, ta);
        mArcWidth = ta.getDimensionPixelSize(R.styleable.ArcSeekBarView_arc_width, dp2px(DEFAULT_ARC_WIDTH));
        mOpenAngle = ta.getFloat(R.styleable.ArcSeekBarView_arc_open_angle, DEFAULT_OPEN_ANGLE);
        mRotateAngle = ta.getFloat(R.styleable.ArcSeekBarView_arc_rotate_angle, DEFAULT_ROTATE_ANGLE);
        mMaxValue = ta.getInt(R.styleable.ArcSeekBarView_arc_max, DEFAULT_MAX_VALUE);
        mMinValue = ta.getInt(R.styleable.ArcSeekBarView_arc_min, DEFAULT_MIN_VALUE);
        // 如果用户设置的最大值和最小值不合理，则直接按照默认进行处理
        if (mMaxValue <= mMinValue) {
            mMaxValue = DEFAULT_MAX_VALUE;
            mMinValue = DEFAULT_MIN_VALUE;
        }
        int progress = ta.getInt(R.styleable.ArcSeekBarView_arc_progress, mMinValue);
        setProgress(progress);
        mBorderWidth = ta.getDimensionPixelSize(R.styleable.ArcSeekBarView_arc_border_width, dp2px(DEFAULT_BORDER_WIDTH));
        mBorderColor = ta.getColor(R.styleable.ArcSeekBarView_arc_border_color, DEFAULT_BORDER_COLOR);

        mThumbColor = ta.getColor(R.styleable.ArcSeekBarView_arc_thumb_color, DEFAULT_THUMB_COLOR);
        mThumbRadius = ta.getDimensionPixelSize(R.styleable.ArcSeekBarView_arc_thumb_radius, dp2px(DEFAULT_THUMB_RADIUS));
        mThumbShadowRadius = ta.getDimensionPixelSize(R.styleable.ArcSeekBarView_arc_thumb_shadow_radius, dp2px(DEFAULT_THUMB_SHADOW_RADIUS));
        mThumbShadowColor = ta.getColor(R.styleable.ArcSeekBarView_arc_thumb_shadow_color, DEFAULT_THUMB_SHADOW_COLOR);
        mThumbWidth = ta.getDimensionPixelSize(R.styleable.ArcSeekBarView_arc_thumb_width, dp2px(DEFAULT_THUMB_WIDTH));
        mThumbMode = ta.getInt(R.styleable.ArcSeekBarView_arc_thumb_mode, THUMB_MODE_STROKE);

        mShadowRadius = ta.getDimensionPixelSize(R.styleable.ArcSeekBarView_arc_shadow_radius, dp2px(DEFAULT_SHADOW_RADIUS));
        ta.recycle();
    }
    ...
    private int[] getArcColors(Context context, TypedArray ta) {
        int[] ret;
        int resId = ta.getResourceId(R.styleable.ArcSeekBarView_arc_colors, 0);
        if (0 == resId) {
    ...
    private class OnClickListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            // 事件回调
            if (null != mOnProgressChangeListener) {
                mOnProgressChangeListener.onProgressChanged(ArcSeekBarView.this, getProgress(), true);
                mOnProgressChangeListener.onStopTrackingTouch(ArcSeekBarView.this);
            }
            return true;
        }
    }
    ...
    public interface OnProgressChangeListener {
         ...
         */
        void onProgressChanged(ArcSeekBarView seekBar, int progress, boolean isUser);
         ...
         * @param seekBar 拖动条
         */
        void onStartTrackingTouch(ArcSeekBarView seekBar);
         ...
         * @param seekBar 拖动条
         */
        void onStopTrackingTouch(ArcSeekBarView seekBar);
    }
    // endregion -----------------------------------------------------------------------------------
}
